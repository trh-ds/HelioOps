...
export default function StormDetailPage() {
      ...
      try {
        ...
        if (active) {
          if (err instanceof ApiError && err.status === 404) {
            ...
            const msg = err instanceof ApiError ? err.message : "Failed to load result"
            setError(msg)
            addToast(msg)
          }
        }
  ...
  async function handleRunPipeline() {
    ...
    try {
      ...
      const msg = err instanceof ApiError ? err.message : "Pipeline failed"
      setRunError(msg)
      addToast(msg)
    } finally {
      setRunning(false)
  ...
  return (
    <DashboardErrorBoundary section={`Storm ${stormId}`}>
      <div className="space-y-6">
        ...
        {!loading && !hasResult && !error && (
          <EmptyState
            ...
            title="No pipeline result yet for this storm."
            action={
              <button
                onClick={handleRunPipeline}
                disabled={running}
